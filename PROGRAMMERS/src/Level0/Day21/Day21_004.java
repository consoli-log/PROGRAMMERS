...
public class Day21_004 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(solution(new String[] {"a", "b", "c"}, new String[] {"c"})); // 2
    ...
    public static int solution(String[] spell, String[] dic) {
        ...
        for (int i = 0; i < spell.length; i++) {
            for (int j = 0; j < dic.length; j++) {
                dic[j] = !dic[j].equals("") && dic[j].contains(spell[i]) ? dic[j].replaceFirst(spell[i], "") : dic[j] + spell[i];
            }
        }
